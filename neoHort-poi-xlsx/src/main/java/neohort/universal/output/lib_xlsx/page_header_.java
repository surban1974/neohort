...
public class page_header_ extends element{
	private static final long serialVersionUID = -569958418345279578L;
	int startY=-1;
public page_header_() {
	super();	
}
...
public void executeLast(Hashtable _tagLibrary, Hashtable _beanLibrary){
	try{	
		...
		if(startY<0)
			startY=0;
		((Sheet)((report_element_base)_beanLibrary.get("SYSTEM:"+iConst.iHORT_SYSTEM_Document)).getContent()).createFreezePane(startY, Y);
		startY=-1;
	}catch(Exception e){
		setError(e,iStub.log_WARN);
	}
