...
Color getField_Color(Class<?> cl, String name, Color d_value) {
	...
	return new Color(result.getRGB());
}
            ...
            if (label != null && label.trim().length() > 0) {
				...
				try{
					...
					while (st.hasMoreTokens()){
						...
						if(cur_el!=null && cur_el.indexOf("SYSTEM_COLOR[")==0)
							colors.add(getField_Color(Color.class,
				...
				if(positionV){
					for(int i=0;i<((chart_dati_LINE_ADVANCED_XY)dati).datiX_st.size();i++){
						if(paint){
							Color curr_color = _prepareColor(i,((chart_dati_LINE_ADVANCED_XY)dati).datiX_st.size());
...
//Line-Bottom
			...
			if(paint){
				...
				cb.setColorFill(new Color(Color.GRAY.getRGB()));
				cb.moveTo(x, y_Line);
			...
			if(paint){
            	if (scale.size() > 0 ) {
	                ...
	                for(int i=0;i<scale.size();i++){
						...
						_prepareline(cb,x,y+i*delta_Scale,x+width-prof,y+i*delta_Scale,5, new Color(Color.gray.getRGB()));
					}
            	}
			}
...
//Scale-Center
        if (dati != null) {
			...
			float delta_minusY = Float.valueOf((String)deltaXYZ.elementAt(1)).floatValue();
				...
				if(delta_minusY<=0){
					cb.setColorFill(new Color(Color.RED.getRGB()));
			...
			while(datiX!=null && datiY!=null){
				Color curr_color = _prepareColor(k,((chart_dati_LINE_ADVANCED_XY)dati).datiX_st.size());
				if(paint && datiX.size()>0 && datiY.size()>0){
					...
					for (int i = 0; i < datiX.size()-1; i++) {
	                	...
	                	if(i>0){
	                		...
	                		double a=Math.sqrt((x1-xprev)*(x1-xprev)+(y1-yprev)*(y1-yprev));
	                		double b=Math.sqrt((x0-xprev)*(x0-xprev)+(y0-yprev)*(y0-yprev));
	                		...
	                		float xM = (xprev+x0)/2;
	                		float yM = (yprev+y0)/2;
	                		double m =Math.sqrt((x0-xM)*(x0-xM)+(y0-yM)*(y0-yM));
	                		double cosgamma = (x0-xM)/m;
	                		if(cosgamma<-1)
	                			cosgamma=-1;
	                		if(cosgamma>1)
	                			cosgamma=1;
	                		double gamma = Math.acos(cosgamma)-beta;
	                		
	                		double m10 = m/5;
	                		if(m10>10) m10=10;
	                		
	                   		float xp=(float)(x0-m10*Math.cos(gamma));
	                		float yp=(float)(y0-m10*Math.sin(gamma));

	                		
		                	cb.setColorStroke(element_color_2);
		                    cb.moveTo(xM, yM);
		                    cb.curveTo(xM, yM,xp,yp, x0, y0);
		                    cb.stroke();


	                		float xM1 = (x0+x1)/2;
	                		float yM1 = (y0+y1)/2;
	                		double m1 =Math.sqrt((xM1-x0)*(xM1-x0)+(yM1-y0)*(yM1-y0));	
	                		double cosgamma1 = (xM1-x0)/m1;
	                		if(cosgamma1<-1)
	                			cosgamma1=-1;
	                		if(cosgamma1>1)
	                			cosgamma1=1;

	                		double gamma1 = Math.acos(cosgamma1)-beta;
	                		
	                		double m1_10=m1/5;
	                		if(m1_10>10) m1_10=10;
	                		
	                   		float xp1=(float)(x0+m1_10*Math.cos(gamma1));
	                		float yp1=(float)(y0+m1_10*Math.sin(gamma1));

	                		
		                	cb.setColorStroke(element_color_2);
		                    cb.moveTo(x0, y0);
		                    cb.curveTo(x0, y0, xp1, yp1,xM1,yM1);
		                    cb.stroke();
