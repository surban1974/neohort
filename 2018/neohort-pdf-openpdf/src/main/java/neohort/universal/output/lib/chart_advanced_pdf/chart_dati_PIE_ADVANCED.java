...
private Vector<Object> getDatiX(float lengthX, float lengthY) {
	...
	Vector<Color> colorsSh = A_chart_content._createColorsShadow(datiX.size());     
	Vector<Color> colors = A_chart_content._createColors(datiX.size());
...
for(int i=sort_result.size()-1;i>-1;i--)
	sort_inv.add(sort_result.elementAt(i));
