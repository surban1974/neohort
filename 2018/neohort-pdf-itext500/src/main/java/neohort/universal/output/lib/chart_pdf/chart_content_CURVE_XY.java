...
public class chart_content_CURVE_XY  extends A_chart_content implements I_chart_content, java.io.Serializable {
...
private float getFun2_A(float x1, float x2, float x3, float y1, float y2, float y3, float A, float B, float C) {
	return (y2-y1-B*(x2-x1))/(x2*x2-x1*x1);
}
private float getFun2_B(float x1, float x2, float x3, float y1, float y2, float y3, float A, float B, float C) {
	return ((y3-y1)*(x2*x2-x1*x1)-(x3*x3-x1*x1)*(y2-y1))/((x3-x1)*(x2*x2-x1*x1)-(x2-x1)*(x3*x3-x1*x1));
}
private float getFun2_C(float x1, float x2, float x3, float y1, float y2, float y3, float A, float B, float C) {
        			...
        			)
{
	...
	double[] x = new double[x_v.size()];
	double[] y = new double[y_v.size()];
	...
	for(int coef=0;coef<x_v.size();coef++){
		x[coef] = Double.valueOf((String)x_v.elementAt(coef)).doubleValue();
		y[coef] = Double.valueOf((String)y_v.elementAt(coef)).doubleValue();
	}

    double one = 1.;
    double zero = 0.;

    int i_ident1, i_ident2;
    ...
    i_ident1 = m;
 	for (j = 0; j < i_ident1; ++j) {
    	r_ident = y[j];
        s = x[j];
        j1 = j - 1;
        if (j1 != 0) {
	        i_ident2 = j1;
 	       for (i_ident = 0; i_ident < i_ident2; ++i_ident) {
  	          r_ident = (s - x[i_ident]) / (r_ident - a[i_ident]);
	        }
    	}
        a[j]=r_ident;
    }
	...
	while(true) {
    	i_ident1 = m1;
    	for (i_ident = 1; i_ident < i_ident1; ++i_ident) {
        	p[i_ident] = zero;
        	q[i_ident] = zero;
    	}
    	i_ident1 = m;
    	for (i_ident = 1; i_ident < i_ident1; ++i_ident) {
        	i2 = i_ident / 2 + 1;
        	i_ident2 = i2;
        	for (j1 = 1; j1 < i_ident2; ++j1) {
            	j = i2 + 2 - j1;
            	r_ident = a[i_ident] * q[j] - x[i_ident - 1] * p[j] + p[j - 1];
            	p[j] = q[j];
            	q[j] = r_ident;
        	}
        	r_ident = a[i_ident] * q[0] - x[i_ident - 1] * p[0];
        	p[0] = q[0];
        	q[0] = r_ident;
    	}
    	if (k == 1) {
	    	i_ident1 = m1;
 	   		for (i_ident = 0; i_ident < i_ident1; ++i_ident) {
  	    		c[i_ident] = q[i_ident];
    		}

		for(int coef=0;coef<a.length;coef++)
			a_v.addElement(new java.math.BigDecimal(a[coef]));
		for(int coef=0;coef<b.length;coef++)
			b_v.addElement(new java.math.BigDecimal(b[coef]));
		for(int coef=0;coef<c.length;coef++)
			c_v.addElement(new java.math.BigDecimal(c[coef]));
		for(int coef=0;coef<p.length;coef++)
			p_v.addElement(new java.math.BigDecimal(p[coef]));
		for(int coef=0;coef<q.length;coef++)
			q_v.addElement(new java.math.BigDecimal(q[coef]));
...
Vector<Object> a_v = new Vector<Object>();
Vector<Object> b_v = new Vector<Object>();
Vector<Object> c_v = new Vector<Object>();
Vector<Object> p_v = new Vector<Object>();
Vector<Object> q_v = new Vector<Object>();
ii23d_c(	datiX.size(),
			datiX,
			datiY,
			a_v,
...
float defer = 0;
                	for (int i = 1; i < datiX.size()-2; i++) {
	                	float x0 = x+Float.valueOf((String)datiX.elementAt(i-1)).floatValue()-delta_minusX;
	                	float y0 = y+Float.valueOf((String)datiY.elementAt(i-1)).floatValue()-delta_minusY;
	                	float x1 = x+Float.valueOf((String)datiX.elementAt(i)).floatValue()-delta_minusX;
	                	float y1 = y+Float.valueOf((String)datiY.elementAt(i)).floatValue()-delta_minusY;
	                	float x2 = x+Float.valueOf((String)datiX.elementAt(i+1)).floatValue()-delta_minusX;
	                	float y2 = y+Float.valueOf((String)datiY.elementAt(i+1)).floatValue()-delta_minusY;
	                	float x3 = x+Float.valueOf((String)datiX.elementAt(i+2)).floatValue()-delta_minusX;
	                	float y3 = y+Float.valueOf((String)datiY.elementAt(i+2)).floatValue()-delta_minusY;

	                	float A0=0;
						float B0=0;
						float C0=0;
						B0 = getFun2_B(x0,x1,x2,y0,y1,y2,A0,B0,C0);
						A0 = getFun2_A(x0,x1,x2,y0,y1,y2,A0,B0,C0);
						C0 = getFun2_C(x0,x1,x2,y0,y1,y2,A0,B0,C0);

	                	float A1=0;
						float B1=0;
						float C1=0;
						B1 = getFun2_B(x1,x2,x3,y1,y2,y3,A1,B1,C1);
						A1 = getFun2_A(x1,x2,x3,y1,y2,y3,A1,B1,C1);
						C1 = getFun2_C(x1,x2,x3,y1,y2,y3,A1,B1,C1);
...
float def1 = 2*A0*x1*B0;def1+=0;
float def2 = 2*A1*x1*B1;def2+=0;
	if(def1-defer<def2-defer){
		A=A0;
		B=B0;
		C=C0;
	}else{
...
//						B = getFun2_B(x1,x12,x2,y1,y12,y2,A,B,C);
//						A = getFun2_A(x1,x12,x2,y1,y12,y2,A,B,C);
//						C = getFun2_C(x1,x12,x2,y1,y12,y2,A,B,C);

						float deltaCurve = 0.1f;
						float xC = x1;
						while(xC<x2){
							float yC=0;
							float xC1=xC+deltaCurve;
							float yC1=0;

							yC=A0*xC*xC+B0*xC+C0;
							yC1=A0*xC1*xC1+B0*xC1+C0;
      						cb.setColorFill(new BaseColor(BaseColor.LIGHT_GRAY.getRGB()));
         	 					cb.moveTo(xC,yC);
       	    					cb.lineTo(xC1,yC1);
        						cb.fill();
							yC=A1*xC*xC+B1*xC+C1;
							yC1=A1*xC1*xC1+B1*xC1+C1;
      						cb.setColorFill(new BaseColor(BaseColor.YELLOW.getRGB()));
         	 					cb.moveTo(xC,yC);
       	    					cb.lineTo(xC1,yC1);
        						cb.fill();

							yC=A*xC*xC+B*xC+C;
							yC1=A*xC1*xC1+B*xC1+C;
