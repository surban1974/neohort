...
import neohort.log.service.log_service;
import neohort.util.util_reflect;

public class iHortExceptionService extends Exception {
	private static final long serialVersionUID = -3917282490455112971L;
	private static String levels=";DEBUG;INFO;WARN;ERROR;FATAL;";

public iHortExceptionService(String mess, Exception ex,  String level) {
	...
	try{
		...
		log_service.writeLog(_log,log_service.getLogInit().get_LogLevel());
	}catch(Exception e){
	}catch(Throwable t){
...
public iHortExceptionService(String mess, Throwable t,  String level) {
	if(!checkLevel(level)) return;
	String _log="["+level+"] ";
		_log+="["+util_reflect.prepareClassInfo(new String[]{"iHort.java","report_element_base.java"},new String[]{"setError","setError"})+"] ";
...
public iHortExceptionService() {
	...
	try{
		...
		log_service.writeLog(_log+toString(),"DEBUG");
	}catch(Exception e){
	}	
}
...
public static boolean checkLevel(String level) {
	...
	try{
		curr_level = log_service.getLogInit().get_LogLevel();
	}catch(Exception e){
	}catch(Throwable e){		
	}
