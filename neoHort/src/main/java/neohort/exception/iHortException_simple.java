...
public iHortException_simple(String mess, Exception ex, String level) {
	...
	try{
		log.writeLog(_log,log.getLogInit().get_LogLevel());
	}catch(Exception e){
	}
	
}
public iHortException_simple(String mess, Throwable t, String level) {
	if(!checkLevel(level)) return;
	String _log="["+level+"] ";
		_log+="["+util_reflect.prepareClassInfo(new String[]{"iHort.java","report_element_base.java"},new String[]{"setError","setError"})+"] ";
